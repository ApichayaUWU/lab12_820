...
int drawCard(void){
	...
	int drawCard = rand()%13 + 1;
	return drawCard;
}

int calScore(int x,int y,int z){
	//Write the function to calculate the score with is the rightmost digit of summation of the scores from three cards.
	int sx = cardScores[x] , sy = cardScores[y] , sz = cardScores[z]; 
	//Return the score. 
	int sum = sx + sy +sz;
	string sum_to_string = to_string(sum);
	int l = sum_to_string.size()-1;
	string rdigit ;
	rdigit = sum_to_string[l];
	return stoi(rdigit);
}

int findYugiAction(int s){	
	int destiny = rand()% 100 +1;
	if(s == 9) return 2; // Yugi will definitely stay (2) when current score (s) is equal to 9
		...
		if(destiny >=1 and destiny <= 69) return 1;
		else return 2;
